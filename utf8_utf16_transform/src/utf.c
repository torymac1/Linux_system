...
get_encoding_function()
{
  ...
  switch (translate) {
    case utf8_to_utf16le:
      return from_utf8_to_utf16le;      //from_utf8_to_utf16le()
...
check_bom()
{
  ...
  if (LOWER_TWO_BYTES(bom) == UTF16LE) {   //if (LOWER_TWO_BYTES(bom) == "UTF16LE") {
...
utf16_glyph_to_code_point(utf16_glyph_t *glyph)
{
  ...
  if(!is_upper_surrogate_pair(*glyph)) {
    ...
    ret = (((glyph->upper_bytes - 0xD800) << 10 ) |      //ret = (((glyph->upper_bytes - 0xD800) << 100) |
...
{
  return (code_point >= 0x10000);    //return (code_point >= 10000);
}