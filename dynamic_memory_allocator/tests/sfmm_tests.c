...
Test(sf_memsuite_student, malloc_several_times, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(300);
	void *y = sf_malloc(5000);
	sf_header *header_x = (sf_header*)((char*)x - 8);
	sf_header *header_y = (sf_header*)((char*)y - 8);

	cr_assert(header_x->block_size << 4 == 320, "Malloc'ed block size not what was expected!");

	cr_assert(header_y->block_size << 4 == 5024, "Malloc'ed block size not what was expected!");

}

Test(sf_memsuite_student, malloc_and_free, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(300);
	void *y = sf_malloc(5000);
	void *z = sf_malloc(13);
	sf_header *header_x = (sf_header*)((char*)x - 8);
	sf_header *header_y = (sf_header*)((char*)y - 8);
	sf_header *header_z = (sf_header*)((char*)z - 8);

	sf_free(y);
	cr_assert(header_y -> block_size <<4 == 5024, "Free block size not what was expected!");
	cr_assert(header_y -> allocated == 0, "Allocated bit is not set!");
	sf_free(x);
	cr_assert(header_x -> block_size <<4 == 5024 + 320, "Free block size not what was expected!");

	cr_assert(header_z->block_size << 4 == 32, "Malloc'ed block size not what was expected!");
...
Test(sf_memsuite_student, realloc_size_zero, .init = sf_mem_init, .fini = sf_mem_fini) {
	...
	//sf_header *header_z = (sf_header*)((char*)z - 8);
	cr_assert(z == NULL, "Reallocated bit is not set, free fail!");
	//cr_assert(header_x -> requested_size == 0, "requested_size is not set, free fail!");

	cr_assert(header_y -> block_size <<4 == 5024, "Malloc size not what was expected!");


}


Test(sf_memsuite_student, free, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(300);
	...
	sf_header *xx = (sf_header*)(x - 8);
	sf_header *yy = (sf_header*)(y - 8);
	sf_header *zz = (sf_header*)(z - 8);

	cr_assert(xx -> allocated == 0, "Allocated bit not 0");
	cr_assert(yy -> allocated == 0, "Allocated bit not 0");
	cr_assert(zz -> allocated == 0, "Allocated bit not 0");

}


Test(sf_memsuite_student, realloc_and_free, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(300);
	void *y = sf_malloc(300);
	void *z = sf_malloc(300);

	void *p = sf_realloc(x,907);
	sf_header *pp = (sf_header*)(p - 8);
	sf_blockprint(pp);
	cr_assert(pp ->  block_size <<4 == 928, "Realloc'ed block size not what was expected!");
	...
	cr_assert(yy -> allocated == 0, "Allocated bit not 0");
	cr_assert(zz -> allocated == 0, "Allocated bit not 0");
	cr_assert(pp -> allocated == 0, "Allocated bit not 0");
